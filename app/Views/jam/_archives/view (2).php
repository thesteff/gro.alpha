...
<script type="text/javascript">
		...
		$.post("<?php echo site_url(); ?>/ajax/join_jam",
			...
			function (return_data) {
				...
				if ($obj['state'] == 1) {
					/*$("#modal_msg").modal({backdrop: true});
					$("#modal_msg .modal-dialog").addClass("default");
					$("#modal_msg .modal-header").html("");
					$("#modal_msg .modal-body").html("<p>"+$obj['data']+"</p>");
					$("#modal_msg").modal('show');*/
	...
	function popup_delete_jam(title,slug) {
		$confirm = "<p>Etes-vous sûr de voulour supprimer la jam '"+title+"' et tous les fichiers qui lui sont associés ?</p>";
		$confirm += "<p style='text-align:center'><input type='button' value='supprimer' onclick='javascript:delete_jam(\""+slug+"\")'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type='button' value='annuler' onclick='javascript:TINY.box.hide()'></p>";
	...
	function delete_jam(slug) {
		TINY.box.hide();
		// Requète ajax au serveur
		$.post("<?php echo site_url(); ?>/ajax/delete_jam",
			...
			function (msg) {	
				if (msg != "success") TINY.box.show({html:msg,boxid:'error',animate:false,width:650});
					...
					// Message de succés // On supprime la jam dans l'UI
					TINY.box.show({html:"Jam supprimée !",boxid:'success',animate:false,width:650, closejs:function(){window.open("<?php echo base_url().'index.php/jam'; ?>","_self");}});
				}
			}
		);
	}
...
<div class="jam panel panel-default row">
	...
	<div class="panel panel-heading panel-bright">
		...
		<div class="title_box">
			...
			<div class="row">
				...
				<!--<?php if (($playlist_item != "null" && (  $jam_item["acces_inscriptions"] > 0  &&  $jam_item['is_archived'] == false ))   ||  ( $this->session->userdata('admin') == 1  ||  $is_admin  ) ): ?>
					<div id="inscr_link" class="green_alert" style="text-align:center; margin-left:20; display:<?php if ($attend || ( $this->session->userdata('admin') == 1  ||  $is_admin  )) echo "block"; else echo "none" ?>">
						<?php
							if ( ( $jam_item["acces_inscriptions"] == 0 || $jam_item['is_archived'] || $playlist_item['list'] == 0) &&  ( $this->session->userdata('admin') == 1  ||  $is_admin  ) )
								$hyperlink = "--== Accès spécial admin ==--";
							else $hyperlink = "Inscription aux morceaux";
							echo "<a href='".site_url()."/jam/inscriptions/".$jam_item['slug']."'>".$hyperlink."</a>";
						?>
					</div>!-->
				<?php endif; ?>
				...
				<?php if (!$jam_item['is_archived']) :?>
					...
					<div class="action_bar btn-group btn-group-sm">
						...
						<?php if (!$attend && !$attend_stage) :?>
							...
							<!-- STAGE Bouton d'inscription au stage -->
				...
				<div class="action_bar btn-group btn-group-sm">
					<!-- MANAGE PARTICIPANTS -->
					<button class="btn btn-default" onclick="location.href='<?php echo site_url();?>/jam/manage/<?php echo $jam_item['slug'] ?>'"><img style="height: 13px; vertical-align:text-top;" src="/images/icons/group.png" alt="Participants">&nbsp;&nbsp;Participants</button>
					<!-- AFFECTATIONS -->
					<button class="btn btn-default" onclick="location.href='<?php echo site_url();?>/jam/affect/<?php echo $jam_item['slug'] ?>'"><img style="height: 15px; vertical-align:text-top;" src="/images/icons/affectation.png" alt="Affectations">&nbsp;&nbsp;Affectations</button>
				</div>	
				<?php endif ?>
				...
				<div class="action_bar btn-group btn-group-sm">
					<!-- MODIFIER -->
					<button class="btn btn-default" href="<?php echo site_url();?>/jam/update/<?php echo $jam_item['slug'] ?>" data-remote="false" data-toggle="modal" data-target="#updateModal"><img style="height: 15px; vertical-align:text-top;" src="/images/icons/edit.png" alt="Modifier">&nbsp;&nbsp;Modifier</button>
					<!-- AFFECTATIONS -->
					<button class="btn btn-default" onclick="location.href='javascript:popup_delete_jam('<?php echo str_replace("'", "\'", $jam_item['title']).'\',\''.$jam_item['slug']; ?>')"><img style="height: 15px; vertical-align:text-top;" src="/images/icons/trash.png" alt="Supprimer">&nbsp;&nbsp;Supprimer</button>
				</div>	
				<?php endif ?>
	...
	<div class="row subPanel">
		...
		<div class="col-lg-5">
			...
			<!-- **** LIEU **** !-->
			<div id="lieu_infos" class="panel panel-default no-border" style="display:flex;">
				<!-- Picto !-->
			...
			<!-- **** PLANNING INFOS **** !-->
			<div id="time_infos" class="panel panel-default no-border" style="display:flex;">
				<!-- Picto !-->
		...
		<!-- ********* SUB PANEL RIGHT ******** !-->
		<div class="col-lg-7 <?php if(!$jam_item['text_html']) echo "hidden" ?>">
			<!-- **** TEXT **** !-->
			<div class="panel panel-body panel-default no-border small">
				<p id="text" style="overflow:hidden; display:none; transition: height 400ms"><?php echo $jam_item['text_html']; ?></p>
			</div>
		</div>
...
<div class="main row">
	...
	<div class="panel panel-default">
		...
		<?php if ($playlist_item != "null" && $playlist_item['list'] != 0): ?>
			...
			<div class="panel panel-heading">
				Liste des titres
				<!-- /* admin */ --><?php if ($this->session->userdata('admin') == "1") echo ' - '.$playlist_item['infos']['title'] ?>
			</div>
			
			<!-- **** LISTE DES MORCEAUX **** !-->
			<div class="panel-body">
				<table class="listTab is_playable">
					<thead>
						<tr>
							<th></th>
							<th width="10" style="text-align:center"><span class="choeurs"><img style='height: 12px;' src='/images/icons/heart.png' title='choeurs'></span></th>
							<th width="10" style="text-align:center"><span class="cuivres"><img style='height: 16px; margin:0 2' src='/images/icons/tp.png' title='cuivres'></span></th>
							<?php if ($stage_item != "null"):?>
								<th width="10" style="text-align:center"><span class="stage"><img style='height: 16px;' src='/images/icons/metro.png' title='réservé aux stagiaires'></span></th>
							<?php endif; ?>
						</tr>
					</thead>
					<tfoot>
						<tr>
							...
							<?php if ($stage_item != "null"):?>
								<th width="10" style="text-align:center"><img style='height: 14px; margin:0 2' src='/images/icons/metro.png'><span class="stage"></span></th>
							<?php endif; ?>
						</tr>
					</tfoot>
	...
	<div class="panel panel-default">
		...
		<div id="benevole" style="font-size: 85%; display:<?php echo ($attend && !$jam_item['is_archived'] && $jam_item['appel_benevole']) ? "block" : "none" ; ?>">
			<?php if ($this->session->userdata('logged')): ?>
				<small>
				<table class="block_info soften" style="padding: 10px 10px 0px 10px; width:90%">
					<tr id="buffet" style="display:<?php if (isset($member) && $attend) echo ($member->buffet) ? "block" : "none"; else echo "none"; ?>">
						<td style="width:17"><img style='height: 16px;' src='/images/icons/fork.png' title='Buffet'></td>
					...
					<tr id="billet" style="display:<?php if (isset($member) && $attend) echo ($member->billet) ? "block" : "none"; else echo "none";  ?>">
						<td style="width:17"><img style='height: 16px;' src='/images/icons/ticket.png' title='Billeterie'></td>
					...
					<tr id="balance" style="display:<?php if (isset($member) && $attend) echo ($member->balance) ? "block" : "none"; else echo "none";  ?>">
						<td style="width:17"><img style='height: 16px;' src='/images/icons/mix.png' title='Balance'></td>
					...
					<tr id="vide" style="display:<?php if (isset($member) && $attend) echo (!$member->buffet && !$member->billet && !$member->balance) ? "block" : "none"; else echo "none";  ?>">
						<td style="width:17"><img style='height: 16px;' src='/images/icons/croche.png' title='Balance'></td>
						<td>Je participe à la soirée en tant que musicien uniquement</td>
					</tr>
					
					<tr>
						<td colspan="2"><div id="update_link" style="float:right; margin-right:-5px"><small><a href="javascript:update_task()">modifier</a></div></td>
					</tr>
				</table>
		...
		<div class="panel-heading">
			Liste des participants <small><?php if (($this->session->userdata('logged'))) echo '(<span id="nb_jammeur">'.sizeof($list_members).'</span>)' ?></small>
		</div>
		...
		<div id="member_list" class="block_info" style="font-size: 85%;">
			...
			<?php if ($this->session->userdata('logged')) : ?>
				...
				<div class="small_block_menu">
					<a id="cat_icon" href='javascript:change_display("cat")'><img style='height: 14px;' src='/images/icons/cat.png' title='afficher par categories'></a>
					<a id="list_icon" href='javascript:change_display("list")' style='display:none'><img style='height: 14px;' src='/images/icons/list.png' title='afficher la liste'></a>
				</div>

				<!-- liste des participants par orde alphabétique !-->
				<ul id="list_alpha" style='list-style-type: none; padding:0px; cursor:default'>
				...
				<ul id="list_cat" style='list-style-type: none; padding:0px; cursor:default; display:none'>
					<?php foreach ($cat_instru_list as $cat): ?>
						<li id="<?php echo $cat['name']; ?>">
							...
							<ul class="cat_content" style='list-style-type: none; padding:0px; margin-top:-10px'>
								<?php foreach ($list_members as $tmember) {
