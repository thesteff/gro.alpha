...
<!-- <script type="text/javascript" src="<?php echo base_url();?>ressources/script/vue-sortable.js"></script> !-->

<!-- Pour rendre Sortable JS compatible avec vuejs !-->
<script>
...
<script type="text/javascript">
		...
		$("input:checkbox").each(function() {	
			...
			$(this).click(function() {
				...
				// On retire la song de la choice_list
					...
					$("#choice_list tr[versionId="+versionId+"][idInstru="+idInstru+"]").remove();
	...
	function add_list_elem(versionId, nameSong, idInstru, labelInstru, to_hidden) {
		...
		elemHTML = "<tr versionId='"+versionId+"' idInstru='"+idInstru+"'><td>";
		//elemHTML += "<span class='song_title'>"+nameSong+"</span>";
		elemHTML += nameSong;
		elemHTML += "<span class='note'> - "+labelInstru+"</span>";
		elemHTML += "</td></tr>";
		
		// On insère la tr dans la choice list
...
<table id="listGroup" class="listTab unactive bright_bg" style="width:550;">
		<tbody v-sortable="{ onUpdate: onUpdate, animation: 150 }">
			<tr v-for="item in list"><td>{{ item.html }} <span class="note">- {{ item.idInstru }}</span></td></tr>
			<tr><td>{{ list | json }}</td></tr>
		</tbody>
</table>
	...
	<div style="display:flex; align-items: flex-start;">
		...
		<div class="block_left">
			...
			<?php /*echo form_open('jam/inscriptions/'.$jam_item['slug']) */?>
				...
				<table id="choiceList" class="listTab unactive bright_bg" style="width:550;">
					<tbody id="choice_list" class="sortable">
					</tbody>
				</table>
				...
				<br />
				<input class="button" onclick="toto()" type="submit" name="submit" value="  Inscription  " />
			</form>	
...
<script>
	...
	var vue = new Vue({
		...
		data: {
			//list: ['Foo', 'Bar', 'Baz']
			/*list: [ 
				{ "html": "Bonjour", "versionId": "3", "idInstru": "16" },
				{ "html": "Coucou", "versionId": "5", "idInstru": "5" },
				{ "html": "Caca", "versionId": "30", "idInstru": "160" }
			]*/
			list: [
				<?php
					////// On insère dans la hidden_select list la liste des morceaux sur lequel le membre est inscrit
					...
					if (isset($keys)) {
						...
						foreach ($keys as $key) {
							//echo "<option versionId='".$inscriptions[$key]['versionId']."'idInstru='".$inscriptions[$key]['instruId']."' selected>".$inscriptions[$key]['versionId']." - ".$inscriptions[$key]['instruId']."</option>";
							//echo "{ 'html': 'coucou', 'versionId': '".$inscriptions[$key]['versionId']."', 'idInstru': '".$inscriptions[$key]['instruId']."' }";
							//$songTitle = get_songTitle($inscriptions[$key]['versionId']);
							//echo "{ 'html': '".$songTitle."' }, ";
						}
					}
				?>
