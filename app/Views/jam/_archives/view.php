...
<?php if ($this->config->item("bootstrap") == true): ?>
	...
	<div class="main_block">
			...
			<?php if (!$attend && !$attend_stage && !$jam_item['is_archived']) :?>
				...
				<div id="join_btn">
					<transition name="fade">
						<button v-on:click="participe" v-if="show">PARTICIPER A LA JAM</button>
					</transition>
				</div>
			<?php endif;?>
		...
		<div style="width:551; max-width:630px;">
			...
			<div id="inscr_link">
				<transition name="fade">
					<div v-html="hyperlink" v-if="show" class="green_alert" style="text-align:center; margin-left:20;"></div>
				</transition>
			</div>
			...
			<?php if ($playlist_item != "null" && $playlist_item['list'] != 0): ?>
				<div class="block_list_title soften">Liste des titres
					...
					<table class="listTab is_playable" style="width:550;">
						<thead>
							<tr>
								<th></th>
	...
	<script>
		...
		var join_btn = new Vue({
			el: '#join_btn',
			data: {
				show: true
			},
			methods: {
				participe: function(event) {
					inscription()
				}
			}
		...
		var inscr_link = new Vue({
			el: '#inscr_link',
			data: {
				show: <?php if (($playlist_item != "null" && (  $jam_item["acces_inscriptions"] > 0  &&  $jam_item['is_archived'] == false ) && $attend)   ||  ( $this->session->userdata('admin') == 1  ||  $is_admin  ) ) echo json_encode(true); else echo json_encode(false) ?>,
				hyperlink: <?php
					if ( ( $jam_item["acces_inscriptions"] == 0 || $jam_item['is_archived'] || $playlist_item['list'] == 0 || !$attend ) &&  ( $this->session->userdata('admin') == 1  ||  $is_admin  ) )
						$hyperlink = "--== Accès spécial admin ==--";
					else $hyperlink = "Inscription aux morceaux";
					$elem = "<a href='".site_url()."/jam/inscriptions/".$jam_item['slug']."'>".$hyperlink."</a>";
